...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public virtual async Task<List<AllProductListModel>> PrepareProductListModelAsync(ProductSearchModel searchModel)
        {
            try
            {
                List<AllProductListModel> allProductsList = new List<AllProductListModel>();
                var dataSettings = DataSettingsManager.LoadSettings();
                SqlConnection con = new SqlConnection(dataSettings.ConnectionString);
                SqlCommand cmd = new SqlCommand("GetAllProductWithSpecificationAttribute", con);
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        AllProductListModel obj = new AllProductListModel();
                        ...
                        obj.StockQuantity = Convert.ToInt32(ds.Tables[0].Rows[i]["StockQuantity"]);
                        obj.Published = Convert.ToBoolean(ds.Tables[0].Rows[i]["Published"]);
                        ...
                        obj.Resistivity= ds.Tables[0].Rows[i]["Resistivity"].ToString();
                        obj.Flat = ds.Tables[0].Rows[i]["Flat"].ToString();
                        obj.FullDescription= ds.Tables[0].Rows[i]["FullDescription"].ToString();

                        Int32 pictureId = ds.Tables[0].Rows[i]["PictureId"].ToString()!=null && ds.Tables[0].Rows[i]["PictureId"].ToString() != String.Empty ? Convert.ToInt32(ds.Tables[0].Rows[i]["PictureId"]) : 0;
                        string fileName = ds.Tables[0].Rows[i]["SeoFilename"].ToString();
                        if (fileName != String.Empty && fileName != null)
                        {
                            string[] splitValues = ds.Tables[0].Rows[i]["MimeType"].ToString().Split("/");
                            string thumbFileName = !string.IsNullOrEmpty(fileName) ? $"{pictureId:0000000}_{fileName}.{splitValues[1]}" : $"{pictureId:0000000}.{splitValues[1]}";

                            obj.PictureThumbnailUrl ="/"+ NopMediaDefaults.ImageThumbsPath + "/" + thumbFileName;
                        }
                        else
                        {
                            obj.PictureThumbnailUrl ="/images/"+ NopMediaDefaults.DefaultImageFileName;
                        }
                ...
                //var products = await _productService.SearchAllProductsAsync(showHidden: true,
                ...
                //    List<AllProductListModel> pModelList =new List<AllProductListModel>();


                //    foreach (var product in products)
                //    {
                //        AllProductListModel objProductModel = new AllProductListModel();
                //        var defaultProductPicture = (await _pictureService.GetPicturesByProductIdAsync(product.Id, 1)).FirstOrDefault();
                //        (objProductModel.PictureThumbnailUrl, _) = await _pictureService.GetPictureUrlAsync(defaultProductPicture, 75);
                //        objProductModel.Id = product.Id;
                //        objProductModel.Name = product.Name;
                //        objProductModel.Sku = product.Sku;
                //        objProductModel.Price = product.Price;
                //        objProductModel.StockQuantity = product.StockQuantity;
                //        objProductModel.Published= product.Published;
                //        objProductModel.FullDescription= product.FullDescription;


                //        List<ProductSpecificationAttributes> productSpecificationAttributesList = new List<ProductSpecificationAttributes>();

                //        //var sp =await PrepareProductSpecificationModelAsync(product);
                //        //var sp =await GetProductSpecificationAttributesAsync(product.Id);
                //        var sp =GetProductSpecificationAttributesAsync(product.Id);
                //        //ProductSpecificationAttributes obj = new ProductSpecificationAttributes();
                //        //foreach (var attr in sp.Groups)
                //        //{
                //        //    //get attribute
                //        //    obj.Diameter = attr.Attributes.Where(x => x.Name.Contains("Diameter")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Diameter")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Type = attr.Attributes.Where(x => x.Name.Contains("Type")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Type")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Dopant = attr.Attributes.Where(x => x.Name.Contains("Dopant")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Dopant")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Orientation = attr.Attributes.Where(x => x.Name.Contains("Orientation")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Orientation")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.ResistivityMin = attr.Attributes.Where(x => x.Name.Contains("Resistivity Min")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Resistivity Min")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.ResistivityMax = attr.Attributes.Where(x => x.Name.Contains("Resistivity Max")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Resistivity Max")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Thickness = attr.Attributes.Where(x => x.Name.Contains("Thickness")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Thickness")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Polish = attr.Attributes.Where(x => x.Name.Contains("Polish")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Polish")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Grade = attr.Attributes.Where(x => x.Name.Contains("Grade")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Grade")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Resistivity = attr.Attributes.Where(x => x.Name.Contains("Resistivity")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Resistivity")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //    obj.Flat = attr.Attributes.Where(x => x.Name.Contains("Flat")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Flat")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //        //}
                //        productSpecificationAttributesList.Add(sp);
                //        objProductModel.ProductSpecificationAttributes = productSpecificationAttributesList;
                //        pModelList.Add(objProductModel);
                //    }
                //    return pModelList;
                //var modelss = new ProductListModel();
                //foreach (var product in products)
                //{
                ...
                //        obj.Flat = attr.Attributes.Where(x => x.Name.Contains("Flat")).Count() > 0 ? attr.Attributes.Where(x => x.Name.Contains("Flat")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First() : String.Empty;
                //    }
                //    productSpecificationAttributesList.Add(obj);
                //    productModel.ProductSpecificationAttributes = productSpecificationAttributesList;
                //    //modelss.Data = productModel;
                //}
                ...
                //        //productModel.FullDescription = product.FullDescription ?? String.Empty;
                ...
                //            //var test1 = attr.Attributes.Where(x => x.Name.Contains("Diameter"));
                //            //var tes1 = attr.Attributes.Where(x => x.Name.Contains("Diameter")).Count();
                //            //var testlist1 = attr.Attributes.Where(x => x.Name.Contains("Diameter")).ToList();
                //            //if (attr.Attributes.Count > 0)
                //            //{
                //            //var testlistr1 = attr.Attributes.Where(x => x.Name.Contains("Diameter")).ToList().Select(x => x.Values.First().ValueRaw.ToString()).First(); 
                //            //    var test = attr.Attributes.Where(x => x.Name.Contains("Diameter"));
                //            //    var tes = attr.Attributes.Where(x => x.Name.Contains("Diameter")).Count();
                //            //    var testlist = attr.Attributes.Where(x => x.Name.Contains("Diameter")).ToList();
                //            //    var t = test.Select(x => x.Values.First().ValueRaw.ToString());
                //            //    var te = test.Select(x => x.Values.First().ValueRaw.ToString()).First();
                //            //    var tests = attr.Attributes.Where(x => x.Name.Contains("Diameter")).FirstOrDefault();
                //            //    if (test != null)
                //            //    {
                //            //        var val = test.Select(x => x.Values.First().ValueRaw.ToString());
                //            //    }

                //            //    string dia = attr.Attributes.Where(x => x.Name.Contains("Diameter")) != null ? attr.Attributes.Select(x => x.Values.First().ValueRaw.ToString()).ToString() : String.Empty;
                //            //    string type = attr.Attributes.Where(x => x.Name.Contains("Type")) != null ? attr.Attributes.Select(x => x.Values.First().ValueRaw.ToString()).ToString() : String.Empty;

                //            //}



                //            //foreach (var r in attr.Attributes)
                //            //{
                //            //    if (r.Name == "Diameter")
                //            //    {
                //            //        obj.Diameter = r.Values.First().ValueRaw.ToString(); 
                //            //    }
                //            //    else if (r.Name == "Type")
                //            //    {
                //            //        obj.Type = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Dopant")
                //            //    {
                //            //        obj.Dopant = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Orientation")
                //            //    {
                //            //        obj.Orientation = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Resistivity Min")
                //            //    {
                //            //        obj.ResistivityMin = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Resistivity Max")
                //            //    {
                //            //        obj.ResistivityMax = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Thickness")
                //            //    {
                //            //        obj.Thickness = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Polish")
                //            //    {
                //            //        obj.Polish = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Grade")
                //            //    {
                //            //        obj.Grade = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Resistivity")
                //            //    {
                //            //        obj.Resistivity = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //    else if (r.Name == "Flat")
                //            //    {
                //            //        obj.Flat = r.Values.First().ValueRaw.ToString();
                //            //    }
                //            //}
                //        }
                //        productSpecificationAttributesList.Add(obj);
        ...
        private async Task<Nop.Web.Models.Catalog.ProductSpecificationModel> PrepareProductSpecificationModelAsync(Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        /// </returns>
        private async Task<IList<Nop.Web.Models.Catalog.ProductSpecificationAttributeModel>> PrepareProductSpecificationAttributeModelAsync(Product product, SpecificationAttributeGroup group)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        //private async Task<ProductSpecificationAttributes> GetProductSpecificationAttributesAsync(int productId)
        //private ProductSpecificationAttributes GetProductSpecificationAttributesAsync(int productId)
        //{
        //    try
        //    {
        //        ProductSpecificationAttributes obj = new ProductSpecificationAttributes();
        //        var dataSettings =Nop.Data.DataSettingsManager.LoadSettings();
        //        System.Data.SqlClient.SqlConnection con = new System.Data.SqlClient.SqlConnection(dataSettings.ConnectionString);
        //        System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand("SP_GetProductSpecificationAttribute", con);
        ...
        //            //System.Data.DataTable dts = ds.Tables[0];
        //            //var te= dts.Select("Diameter");


        //            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        //            {
        //                string attr = ds.Tables[0].Rows[i]["Name"].ToString();
        //                switch (attr)
        //                {
        //                    case "Diameter":
        //                        obj.Diameter = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Type":
        //                        obj.Type = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Dopant":
        //                        obj.Dopant = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Orientation":
        //                        obj.Orientation = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "ResistivityMin":
        //                        obj.ResistivityMin = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "ResistivityMax":
        //                        obj.ResistivityMax = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Thickness":
        //                        obj.Thickness = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Polish":
        //                        obj.Polish = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Grade":
        //                        obj.Grade = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Resistivity":
        //                        obj.Resistivity = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                    case "Flat":
        //                        obj.Flat = ds.Tables[0].Rows[i]["Value"].ToString();
        //                        break;
        //                        //default:
        //                        //    throw new Exception();
        //                }

        //                //obj.Diameter = ds.Tables[0].Rows[i]["Name"].ToString() == "Diameter" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Diameter;
        //                //obj.Type = ds.Tables[0].Rows[i]["Name"].ToString() == "Type" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Type;
        //                //obj.Dopant = ds.Tables[0].Rows[i]["Name"].ToString() == "Dopant" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Dopant;
        //                //obj.Orientation = ds.Tables[0].Rows[i]["Name"].ToString() == "Orientation" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Orientation;
        //                //obj.ResistivityMin = ds.Tables[0].Rows[i]["Name"].ToString() == "ResistivityMin" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.ResistivityMin;
        //                //obj.ResistivityMax = ds.Tables[0].Rows[i]["Name"].ToString() == "ResistivityMax" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.ResistivityMax;
        //                //obj.Thickness = ds.Tables[0].Rows[i]["Name"].ToString() == "Thickness" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Thickness;
        //                //obj.Polish = ds.Tables[0].Rows[i]["Name"].ToString() == "Polish" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Polish;
        //                //obj.Grade = ds.Tables[0].Rows[i]["Name"].ToString() == "Grade" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Grade;
        //                //obj.Resistivity = ds.Tables[0].Rows[i]["Name"].ToString() == "Resistivity" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Resistivity;
        //                //obj.Flat = ds.Tables[0].Rows[i]["Name"].ToString() == "Flat" ? ds.Tables[0].Rows[i]["Value"].ToString() : obj.Flat;
        //            }
