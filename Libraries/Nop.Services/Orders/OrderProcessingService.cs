...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <summary>
        /// This method is used for add quote request
        /// </summary>
        /// <param name="quoteRequest">QuoteRequestModel quoteRequest</param>
        /// <returns>object</returns>
        public async Task<object> AddQuoteRequest(QuoteRequestModel quoteRequest)
        {
            try
            {
            ...
            var details = new PlaceOrderContainer
            {
                ...
                Customer = await _customerService.GetCustomerByIdAsync((await _workContext.GetCurrentCustomerAsync()).Id)
            };
            //var currencyTmp = _currencyService.GetCurrencyById(
            var currencyTmp =await _currencyService.GetCurrencyByIdAsync(await
                //_genericAttributeService.GetAttribute<int>(details.Customer, NopCustomerDefaults.CurrencyIdAttribute, _storeContext.CurrentStore.Id));
                _genericAttributeService.GetAttributeAsync<int>(details.Customer, NopCustomerDefaults.CurrencyIdAttribute , (await _storeContext.GetCurrentStoreAsync()).Id));
            ...
            details.CustomerCurrencyCode = (customerCurrency).CurrencyCode;
            //details.CustomerCurrencyRate = customerCurrency.Rate / primaryStoreCurrency.Rate;
            details.CustomerCurrencyRate =(customerCurrency).Rate /(await primaryStoreCurrency).Rate;
            ...
            var order = new Order
            {
                ...
                CustomerIp = _webHelper.GetCurrentIpAddress(),
                OrderSubtotalExclTax = Convert.ToDecimal(quoteRequest.SubTotal),
                OrderShippingExclTax = Convert.ToDecimal(quoteRequest.ShippingCharge),
                OrderTotal = Convert.ToDecimal(quoteRequest.OrderTotal),
                RefundedAmount = decimal.Zero,
                CustomerCurrencyCode = details.CustomerCurrencyCode,
                CurrencyRate = details.CustomerCurrencyRate,
            ...
            objAddress.FirstName = quoteRequest.FirstName;
            objAddress.LastName = quoteRequest.LastName;
            objAddress.Email = quoteRequest.Email;
            objAddress.Company = quoteRequest.Company;
            objAddress.CountryId = quoteRequest.CountryId;
            objAddress.StateProvinceId = quoteRequest.StateProvinceId;
            objAddress.County = quoteRequest.CountryName;
            objAddress.City = quoteRequest.City;
            objAddress.Address1 = quoteRequest.Address1;
            objAddress.Address2 = quoteRequest.Address2;
            objAddress.ZipPostalCode = quoteRequest.ZipPostalCode;
            objAddress.PhoneNumber = quoteRequest.PhoneNumber;

            //_addressService.InsertAddress(objAddress);
            await _addressService.InsertAddressAsync(objAddress);
            order.BillingAddressId = objAddress.Id;

            //check shipping address is different or same to billing address
            if (quoteRequest.IsShippingAddressSame == false)
            {
                Address obj = new Address();
                obj.FirstName = quoteRequest.FirstNameDSA;
                obj.LastName = quoteRequest.LastNameDSA;
                obj.CountryId = quoteRequest.CountryIdDSA;
                obj.StateProvinceId = quoteRequest.StateIdDSA;
                obj.County = quoteRequest.CountryNameDSA;
                obj.City = quoteRequest.CityDSA;
                obj.Address1 = quoteRequest.Address1DSA;
                obj.Address2 = quoteRequest.Address2DSA;
                obj.ZipPostalCode = quoteRequest.ZipPostalCodeDSA;
                obj.PhoneNumber = quoteRequest.PhoneNumberDSA;

                //_addressService.InsertAddress(obj);
                await _addressService.InsertAddressAsync(obj);
                order.ShippingAddressId = obj.Id;
            }
            else
            {
                order.ShippingAddressId = objAddress.Id;
            }

            order.IsOuoteRequest = true;
            ...
            var cart = _shoppingCartService.GetShoppingCartAsync(await _workContext.GetCurrentCustomerAsync(), ShoppingCartType.ShoppingCart, (await _storeContext.GetCurrentStoreAsync()).Id);
            //var orderItem = AddOrderItems(cart, order);
            var orderItem =await AddOrderItems(await cart, order);
            var message = "Order is inserted!";
            return (new { Order = order, message, OrderId = order.Id, BillingAddressId = order.BillingAddressId, ShippingAddressId = order.ShippingAddressId });
        ...
        public async Task<OrderItem> AddOrderItems(IList<ShoppingCartItem> cart, Order order)
        {
            try
            {
                ...
                foreach (var sc in cart)
                {
                    ...
                    //var scUnitPrice = Convert.ToDecimal(_shoppingCartService.GetUnitPriceAsync(sc));
                    ////var scSubTotal = _shoppingCartService.GetSubTotal(sc, true, out var discountAmount,
                    ////    out var scDiscounts, out _);
                    //var scSubTotal =Convert.ToDecimal(await _shoppingCartService.GetSubTotalAsync(sc, false));
                    //var scUnitPriceInclTax =
                    //    //_taxService.GetProductPrice(product, scUnitPrice, true, _workContext.CurrentCustomer, out var _);
                    //    await _taxService.GetProductPriceAsync(await product, scUnitPrice, true,await _workContext.GetCurrentCustomerAsync());
                    //var scUnitPriceExclTax =
                    //    //_taxService.GetProductPrice(product, scUnitPrice, false, _workContext.CurrentCustomer, out _);
                    //    _taxService.GetProductPriceAsync(await product, scUnitPrice, false,await _workContext.GetCurrentCustomerAsync());
                    //var scSubTotalInclTax =
                    //    //_taxService.GetProductPrice(product, scSubTotal, true, _workContext.CurrentCustomer, out _);
                    //    _taxService.GetProductPriceAsync(await product, scSubTotal, true,await _workContext.GetCurrentCustomerAsync());
                    //var scSubTotalExclTax =
                    //    //_taxService.GetProductPrice(product, scSubTotal, false, _workContext.CurrentCustomer, out _);
                    //    _taxService.GetProductPriceAsync(await product, scSubTotal, false,await _workContext.GetCurrentCustomerAsync());
                    //var discountAmountInclTax =
                    //    //_taxService.GetProductPrice(product, discountAmount, true, _workContext.CurrentCustomer, out _);
                    //    _taxService.GetProductPriceAsync(await product, scSubTotal, true,await _workContext.GetCurrentCustomerAsync());
                    //var discountAmountExclTax =
                    //    _taxService.GetProductPrice(product, discountAmount, false, _workContext.CurrentCustomer, out _);
                    ////foreach (var disc in scDiscounts)
                    ////    if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
                    ////        details.AppliedDiscounts.Add(disc);
                    ...
                    var scUnitPrice = (await _shoppingCartService.GetUnitPriceAsync(sc, true)).unitPrice;
                    var (scSubTotal, discountAmount, scDiscounts, _) = await _shoppingCartService.GetSubTotalAsync(sc, true);
                    var (scUnitPriceInclTax, taxRatePriceIncl )=
                        await _taxService.GetProductPriceAsync(product, scUnitPrice, true,await _workContext.GetCurrentCustomerAsync());
                    var (scUnitPriceExclTax, taxRatePriceExcl )=
                        await _taxService.GetProductPriceAsync(product, scUnitPrice, false, await _workContext.GetCurrentCustomerAsync());
                    var (scSubTotalInclTax, taxRateSubTatalIncl )=
                        await _taxService.GetProductPriceAsync(product, scSubTotal, true, await _workContext.GetCurrentCustomerAsync());
                    var (scSubTotalExclTax ,taxRateSubTatalExcl)=
                        await _taxService.GetProductPriceAsync(product, scSubTotal, false, await _workContext.GetCurrentCustomerAsync());
                    var (discountAmountInclTax, taxRateDiscountAmountInclTax)=
                        await _taxService.GetProductPriceAsync(product, discountAmount, true, await _workContext.GetCurrentCustomerAsync());
                    var (discountAmountExclTax, taxRateDiscountAmountExclTax )=
                        await _taxService.GetProductPriceAsync(product, discountAmount, false, await _workContext.GetCurrentCustomerAsync());
                    //foreach (var disc in scDiscounts)
                    //    if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
                    //        details.AppliedDiscounts.Add(disc);
                    ...
                    //attributes
                    var attributeDescription =
                        //_productAttributeFormatter.FormatAttributes(product, sc.AttributesXml, _workContext.CurrentCustomer);
                       await _productAttributeFormatter.FormatAttributesAsync(product, sc.AttributesXml, await _workContext.GetCurrentCustomerAsync());
                ...
                cart.ToList().ForEach(sci => _shoppingCartService.DeleteShoppingCartItemAsync(sci, false));
                return orderItemObj;
